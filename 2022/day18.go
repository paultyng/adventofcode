...
func runDay18Part1(ctx context.Context, args []string) (string, error) {
	path := "day18.input"
	if len(args) > 0 {
		path = args[0]
	}
	_, err := readInputDay18(path)
	if err != nil {
...
func runDay18Part2(ctx context.Context, args []string) (string, error) {
	path := "day18.input"
	if len(args) > 0 {
		path = args[0]
	}
...
func readInputDay18(path string) ([]rucksack, error) {
