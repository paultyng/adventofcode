...
func runDay24Part1(ctx context.Context, args []string) (string, error) {
	path := "day24.input"
	if len(args) > 0 {
		path = args[0]
	}
	_, err := readInputDay24(path)
	if err != nil {
...
func runDay24Part2(ctx context.Context, args []string) (string, error) {
	path := "day24.input"
	if len(args) > 0 {
		path = args[0]
	}
...
func readInputDay24(path string) ([]rucksack, error) {
