...
func runDay25Part1(ctx context.Context, args []string) (string, error) {
	path := "day25.input"
	if len(args) > 0 {
		path = args[0]
	}
	_, err := readInputDay25(path)
	if err != nil {
...
func runDay25Part2(ctx context.Context, args []string) (string, error) {
	path := "day25.input"
	if len(args) > 0 {
		path = args[0]
	}
...
func readInputDay25(path string) ([]rucksack, error) {
