...
func runDay22Part1(ctx context.Context, args []string) (string, error) {
	path := "day22.input"
	if len(args) > 0 {
		path = args[0]
	}
	_, err := readInputDay22(path)
	if err != nil {
...
func runDay22Part2(ctx context.Context, args []string) (string, error) {
	path := "day22.input"
	if len(args) > 0 {
		path = args[0]
	}
...
func readInputDay22(path string) ([]rucksack, error) {
