...
func runDay17Part1(ctx context.Context, args []string) (string, error) {
	path := "day17.input"
	if len(args) > 0 {
		path = args[0]
	}
	_, err := readInputDay17(path)
	if err != nil {
...
func runDay17Part2(ctx context.Context, args []string) (string, error) {
	path := "day17.input"
	if len(args) > 0 {
		path = args[0]
	}
...
func readInputDay17(path string) ([]rucksack, error) {
